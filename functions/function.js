...
function returnOpenAiReply(question, trainData, model, openAikey) {
...
async function returnSaveObj(k, toName, senderNumber, incomingMsg) {
  let savObj = {};
  const trainData = k?.state?.trainData;
  const model = k?.state?.model;
  const apiKey = k?.state?.apiKey;

  console.log({
    nodeType: k.nodeType,
  });

  if (k.nodeType === "AI_BOT") {
    const openAiReply = await returnOpenAiReply(
      incomingMsg,
      trainData,
      model,
      apiKey
      ...
      if (nodes.length > 0 && edges.length > 0) {
        ...
        if (answer.length > 0) {
          for (const k of answer) {
            const savObj = await returnSaveObj(
              k,
              toName,
              senderNumber,
              incomingMsg
